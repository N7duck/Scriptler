...
-- Roblox'un telefon/cihaz saatinden tamamen bağımsız olan evrensel zaman fonksiyonu
local function getNetworkTime()
    local utcTable = os.date("!*t") -- Ünlem işareti saf UTC saatini zorunlu kılar, cihaz saatini yok sayar
...
ScreenGui.Name = "DuckHubKeySystem"
...
Title.Text = "DUCK HUB - KEY SYSTEM"
...
local BtnCorner1 = Instance.new("UICorner", CheckBtn)
...
local BtnCorner2 = Instance.new("UICorner", GetKeyBtn)

CheckBtn.MouseButton1Click:Connect(function()
    local currentTime = getNetworkTime()
    local expireTime = os.time(ExpirationDate)
    
    if currentTime > expireTime then
        Title.Text = "KEY EXPIRED! GET A NEW ONE"
        ...
        Title.Text = "CORRECT KEY! LOADING..."
        Title.TextColor3 = Color3.fromRGB(0, 255, 0)
        wait(1.5)
        _G.KeyPassed = true -- 🚀 İŞTE EKSİK OLAN KISIM BURASI!
        ScreenGui:Destroy()
        print("Duck Hub Successfully Loaded!")
    else
...
repeat task.wait() until _G.KeyPassed == true

-- ============================================================

-- ============================================================
-- DUCK DUELS V2 - WITH STAND DROP (Crasher) & JUMP DROP
...
if _G.DuckDuelsV2_Running then return end
_G.DuckDuelsV2_Running = true
...
local CONFIG_FILE = "DuckDuelsConfig.json"
local CONFIG_BACKUP = "DuckDuelsConfig.bak"
...
-- ============================================================
local Presets = {}
local PRESET_FILE = "Duck DuelsPresets.json"
local LAST_PRESET_FILE = "DuckDuelsLastPreset.json"
...
-- CLEANUP
do
    local cleanupNames = {"VyseSlottedGUI","VyseAsireGUI","VyseAsireHubV4","VyseAsireHubV5","VyseAsireHubV5_1","AsireHubV5_1","AsireHubV5_2","LaitoHubV1","DuckDuelsV1"}
...
local function Main()
    if _G.DuckDuelsV2_MainExecuted then return end
    _G.DuckDuelsV2_MainExecuted = true

    local gui=Instance.new("ScreenGui")
    gui.Name="DuckDuelsV2"; gui.ResetOnSpawn=false; gui.DisplayOrder=10
    ...
    titleLbl.BackgroundTransparency = 1; titleLbl.Text = "Duck Duels"
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        if _G.DuckDuelsQAHide then pcall(_G.DuckDuelsQAHide, true) end
        requestSave()
    end)
    ...
    local function cleanParticlesAndLights()
        ...
        print("[Duck Duels] Cleaned "..removed.." effects/lights")
    end
    local origLighting = {
    ...
    do
        local page = buildPage("Movement", function()
            ...
            standDropBtn.MouseButton1Click:Connect(function()
                ...
                requestSave()
                print("[Duck Duels] Drop type changed to: Stand Drop (Brainrot fling)")
            end)
            ...
            jumpDropBtn.MouseButton1Click:Connect(function()
                ...
                requestSave()
                print("[Duck Duels] Drop type changed to: Jump Drop (ascend)")
            end)
    ...
    do
        local page = buildPage("Visual", function()
            ...
            makeSkyBtn("Blue Sky","blue"); makeSkyBtn("Duck Sky","duck"); makeSkyBtn("Night Mode","night"); makeSkyBtn("Day Mode","day")
            makeGap(4)
    ...
    do
        local page = buildPage("Settings", function()
            ...
            local fl = Instance.new("TextLabel", fw); fl.Size = UDim2.new(1,0,1,0); fl.BackgroundTransparency=1; fl.Text="Duck Duels  ·  powered by luck 🍀"; fl.TextColor3=Color3.fromRGB(50,100,65); fl.Font=Enum.Font.Gotham; fl.TextSize=10; fl.TextXAlignment=Enum.TextXAlignment.Center
            ...
            _G._VezyFlashSave = function(success)
                ...
                task.delay(1.5,function() if lbl and lbl.Parent then lbl.Text="Duck Duels  ·  powered by luck 🍀"; lbl.TextColor3=Color3.fromRGB(50,100,65) end end)
            end
        end)
        page.LayoutOrder = 6
    end
    ...
    local function getRagTimerLbl()
        ...
        local bb = head:FindFirstChild("DuckDuelsBB"); if not bb then return nil end
    ...
    local function setupChar(char)
        ...
        if head then
            local oldBB=head:FindFirstChild("DuckDuelsBB"); if oldBB then oldBB:Destroy() end
            local bb=Instance.new("BillboardGui", head); bb.Name="DuckDuelsBB"; bb.Size=UDim2.new(0,180,0,100); bb.StudsOffset=Vector3.new(0,3,0); bb.AlwaysOnTop=true
            ...
            local discordLbl=Instance.new("TextLabel",bb); discordLbl.Size=UDim2.new(1,0,0,22); discordLbl.BackgroundTransparency=1; discordLbl.Text="discord.gg/5uYJqJJDBj"; discordLbl.TextColor3=Color3.fromRGB(100,255,100); discordLbl.Font=Enum.Font.GothamBold; discordLbl.TextScaled=true; discordLbl.TextStrokeTransparency=0.1; discordLbl.TextStrokeColor3=Color3.new(0,0,0); discordLbl.LayoutOrder=2
    ...
    RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            ...
            if head2 then
                local bb2=head2:FindFirstChild("DuckDuelsBB")
    ...
    UIS.InputBegan:Connect(function(inp,gp)
        ...
        elseif kc==Keys.guiHide then
            if isKb then
                ...
                if _G.DuckDuelsQAHide then pcall(_G.DuckDuelsQAHide, not State.guiVisible) end
                requestSave()
            end
        end
    end)
    ...
    cloverBtn.Name = "DuckDuelsClover"
    ...
    saveConfig = function()
        ...
        if not success then
            ...
            warn("[Duck Duels] Config save FAILED!")
        else
    ...
    loadConfig = function()
        ...
        if not raw or raw == "" then
            if _isfile(CONFIG_BACKUP) then
                ...
                if raw and raw ~= "" then
                    print("[Duck Duels] Loaded config from backup")
                end
            end
        end
        if not raw or raw == "" then
            print("[Duck Duels] No valid config file found, using defaults")
            return false
        end
        ...
        if not ok or not decErr then
            ...
            warn("[Duck Duels] Corrupt config deleted, using defaults")
            return false
        end
        ...
        print("[Duck Duels] Config loaded successfully")
        return true
    end
    ...
    startAutoSteal()
    print("[Duck Duels] Ready. Stand drop = Brainrot fling (safe). Jump drop = ascend.")
end

-- ============================================================
-- SAFE MAIN EXECUTION
-- ============================================================
if not _G.DuckDuelsV2_MainExecuted then
...
local function setupOtherPlayerSpeed(player)
    ...
    local function onCharacterAdded(char)
        ...
        local oldBB = head:FindFirstChild("DuckDuelsBB_Other")
        if oldBB then oldBB:Destroy() end
        local bb = Instance.new("BillboardGui", head)
        bb.Name = "DuckDuelsBB_Other"
